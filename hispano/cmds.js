		...
		switch ((args[0].replace('*', ''))){
			...
			case 'cmds': case 'commands':
				if (args[1] != undefined){
					...
					const embed = { "title": "***Puedes usar estos comandos solo en este servidor!***", "color": 2672880, "footer": { "text": "Da igual entre mayusculas o minusculas." }, "fields": [ { "name": "**__Raiding:__**", "value": "```fix\nafk; clean; headcount; resetChannel; unlock; location```"}, { "name": "**__Miscellaneous:__**", "value": "```fix\navatar; commands; slurp; ping```\n-> Escribe un mensaje privado a SkrillerGG si necesitas un nuevo comando.\nPara aprender mas sobre un comando, usa el comando *commands <command name>" } ] };
			...
			if (afks[x]['ended'] == false){
				...
				if (afks[x]['timeleft'] <= 0){
					...
					if (afks[x]['postafk'] == false){
						...
						await spt.channels.get(afks[x]['channel']).overwritePermissions(spt.guilds.get(config.hispano.id).roles.find(role => role.name == config.hispano.rdrRole), { 'CONNECT': false, 'SPEAK': false });
					...
					if (afks[x]['postafk'] == true){
						// postafk update
						spt.channels.get(config.hispano.rlChan).fetchMessage(afks[x]['controlpanel'])
...
async function ckDeaf(spt){
	if (spt.channels.get(config.hispano.vc.one).members.size > 0){
		spt.channels.get(config.hispano.vc.one).members.forEach(async function(raiders){
			await isRL(spt, 'hispano', raiders.id).then(async function(value){
				if (raiders.deaf && !warnedDeafs.includes(raiders) && !value){
					raiders.send(`Te acabas de mutear tu mismo en el Raid. si no te desmuteas en los siguientes 30 segundos, seras suspendido! si te muteaste aproposito.`)
					spt.channels.get(config.hispano.rlChan).send(`${raiders} puedes abandonar el canal y abandonar la dungeon o seras suspendido por crash.`);
					warnedDeafs.push(raiders);
				}
			})
		})
	}
	if (spt.channels.get(config.hispano.vc.two).members.size > 0){
		spt.channels.get(config.hispano.vc.two).members.forEach(async function(raiders){
			await isRL(spt, 'hispano', raiders.id).then(async function(value){
				if (raiders.deaf && !warnedDeafs.includes(raiders) && !value){
					raiders.send(`Te acabas de mutear tu mismo en el Raid. si no te desmuteas en los siguientes 30 segundos, seras suspendido! si te muteaste aproposito.`)
					spt.channels.get(config.hispano.rlChan).send(`${raiders} puedes abandonar el canal y abandonar la dungeon o seras suspendido por crash.`);
	...
	if (spt.channels.get(config.hispano.vc.three).members.size > 0){
		spt.channels.get(config.hispano.vc.three).members.forEach(async function(raiders){
			await isRL(spt, 'hispano', raiders.id).then(async function(value){
				if (raiders.deaf && !warnedDeafs.includes(raiders) && !value){
					raiders.send(`Te acabas de mutear tu mismo en el Raid. si no te desmuteas en los siguientes 30 segundos, seras suspendido! si te muteaste aproposito.`)
					spt.channels.get(config.hispano.rlChan).send(`${raiders} puedes abandonar el canal y abandonar la dungeon o seras suspendido por crash.`);
