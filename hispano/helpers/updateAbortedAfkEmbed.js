...
function updateEndedAFK(spt, afkObj){
	...
	spt.channels.get(afkObj['channel']).members.forEach(async function(raiders){
		if (raiders.roles.find(r => raiders.roles.find(r => r.name === config.hispano.rlRole))){
			rlAmount++;
		}
	})
	const embed = {
		...
		author: {
		name: "Fungal Cavern started by "+spt.guilds.get(config.hispano.id).members.get(afkObj['host']).displayName+" in "+channelName,
